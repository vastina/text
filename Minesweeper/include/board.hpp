...
class Drawer
{
  ...
  u32 gap {5};
public:
  Drawer( DrawBoard& b, u32 r, u32 c, u32 window_width, u32 window_height ) : rows( r ), cols( c ), width( window_width ), height( window_height )
  {
    drawer.resize( rows );
    for ( auto& row : drawer )
      row.resize( cols );
    ...
    for ( u32 i{0u};auto& row : drawer ) {
      for ( u32 j{0u}; auto& ts : row ) {
        ...
        ts->setRect(i*unit_width + gap, j*unit_height + gap, unit_width - gap, unit_height - gap);
        j++;
      }
      i++;
    }
